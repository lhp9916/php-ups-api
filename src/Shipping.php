        ...
        if ($shipment->getPaymentInformation()) {
            ...
            for ($shipmentChargeRec = 1; $shipmentChargeRec <= 2; $shipmentChargeRec++) {
                ...
                if ($rec->getBillShipper()) {
                    ...
                    $node = $node->appendChild($xml->createElement('BillReceiver'));

                    $billReceiver = $rec->getBillReceiver();
                    if (isset($billReceiver) && $billReceiver->getAccountNumber()) {
                        $node->appendChild($xml->createElement('AccountNumber', $billReceiver->getAccountNumber()));
                    }
                    $addressNode = $node->appendChild($xml->createElement('Address'));
                    $address = $billReceiver->getAddress();
                    if (isset($address) && $address->getPostalCode()) {
                        $addressNode->appendChild($xml->createElement('PostalCode', $address->getPostalCode()));
                    }
